...
`.split('\n');
  ...
  for (let y=0;y<5;y++) {if (a.length<4) {r+=''.padEnd((width-(a.length*(w+space)-space))/2);} a.forEach((n)=>{if (bn[n*6+y+1]) {r+=bn[n*6+y+1].padEnd(w+space)}});if (y<4) {r+=delimiter}} return r;
}
