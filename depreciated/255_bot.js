...
function messagehandler(data,say) {
	if (/--help/i.test(data)) {say('help: Hi | time | ping | ping off')}
...
var interval;
Date.prototype.addHours= function(h){this.setHours(this.getHours()+(h||0)); return this;}
function get_time(long) {var date=new Date().addHours();var hour=date.getHours(); console.log(date); hour=(hour<10?"0":"")+hour;var min=date.getMinutes();min=(min<10?"0":"")+min;return hour+((long)?":":"")+min;}
