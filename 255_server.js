...
const server = express()
  .get('/255/m/:m', function(req, res) {broadcast(req.params.m);res.send('')})
  .get('/255/api/setpublicip', function(req, res) {var ip = req.headers['x-forwarded-for'] || req.connection.remoteAddress; publicip=ip.replace(/^.*:/, ''); res.send(publicip);})
  .get('/255/api/getpublicip', function(req, res) {res.send(publicip)})
  .get('/255/api/local', function(req, res) {res.send('<HTML><HEAD><META HTTP-EQUIV="refresh" CONTENT="0;URL=http://'+publicip+':8080"></HEAD></HTML>')})
  .get('/255', function(req, res) {res.sendFile(require('path').join(__dirname,'255_client_simple.html'))})
  .get('*', function(req, res) {res.send('404 255_server')})
  .listen(3000);
